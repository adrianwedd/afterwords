...
def test_splits_at_400_char_boundary():
    # ~210 chars each sentence; together >400 chars → must split
    s1 = "A" * 200 + "."
    s2 = "B" * 200 + "."
...
def test_exclamation_and_ellipsis_as_sentence_boundaries():
    ...
    # All together <400 chars → 1 chunk, but splits ARE at these boundaries
    assert len(chunks) >= 1
...
def test_single_sentence_exceeding_max_chars_is_word_split():
    ...
    long_sentence = "word " * 100  # 500 chars, no sentence-end punctuation
    chunks = _run_chunk(long_sentence.strip())
    assert all(len(c) <= 400 for c in chunks), f"Chunk exceeds 400 chars: {[len(c) for c in chunks]}"
    assert len(chunks) >= 2


def test_word_split_preserves_all_words():
    # Ensure no words are dropped when word-splitting a long sentence.
    long_sentence = ("alpha " * 80).strip()  # 480 chars
