...
> [!TIP]
> **Agent Splicing Tool:** Agents should use `python scripts/splice-voice.py --wav <file> --voice <name> --match "<unique substring>" --reject-music` to automate this entire workflow. This script automatically runs whisper, finds segments matching the target actor's dialogue, applies the spectral heuristic, performs noise reduction and fading, and updates the JSON profile seamlessly. When downloading source audio, prefer highly targeted YouTube queries (e.g., `"Character Name specific quote"`) over generic character names to avoid multi-actor scenes.
