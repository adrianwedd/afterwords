...
- **Playback queue: cap, not coalesce** — when more than `MAX_QUEUE` replies are waiting, the oldest are pruned and the rest drain oldest-first. Queue items are named with a nanosecond timestamp so a same-second burst still has a total order.
...
- **Queue coalesce destroyed pending turns** — an earlier revision of this branch spoke only the newest queued turn and deleted every other one, discarding both the audio and the archive record of every reply but the last (`rm -f` ran before synthesis or archiving). With the shared queue used by Claude/Cursor/Gemini/AGy, a burst from two agents lost all but one turn permanently. Restored: prune to `MAX_QUEUE` newest, then drain oldest-first. Measured 30 queued turns: was 1 spoken / 1 archived, now 25 spoken / 25 archived.
- **Queue ordering was not a total order** — item filenames were `$(date +%s)-${RANDOM}.json`, one-second granularity, so two agents finishing in the same second had no defined "newest" and the coalesce could keep the *older* turn and delete the newer one. Filenames now carry a nanosecond timestamp.
- **Codex worker dropped a turn on play-lock contention** — it logged and `continue`d with the item already deleted, while the sibling Claude worker added a capped re-queue in the same release. Ported the `attempts`-capped re-queue.
- **Text sidecar did not match the spoken audio** — the `.txt` sidecar was written from the full reply *before* the optional summarization step, so with summarization on the archive held the whole reply while the MP3s held only the summary. `scripts/audit-archive.py` compares exactly those two, so every summarized turn flagged high WER. The sidecar is now written from the text actually synthesized.
- **URL stripping fused sentences** — `https?://\S+` consumed the punctuation that ended the sentence (`See https://example.com/docs. It works.` → `See It works.`). The trailing punctuation is preserved.
- **Agent-specific `.afterwords` keys lost to global ones** — `parse_afterwords_config` walked the file top-to-bottom and let the last matching line win, so a global `summarize:` below an agent-specific `cursor_summarize:` overrode it, contradicting the function's documented contract. Now parsed in two passes (global, then agent-specific).
- **Summarizer ignored for a Claude main conversation** — the worker passed `--agent ""` (no `.agent_type` on a main turn), so the natural `claude_summarize` key never matched. Defaults to `claude`.
- **Hermes native hook could drop archived chunks** — intermediate-chunk archive jobs were dispatched with an unreferenced `asyncio.create_task(asyncio.to_thread(...))` and never awaited, so they were GC-eligible and could be torn down mid-encode. References are held and gathered before the play lock is released.
