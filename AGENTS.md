...
**Splicing references for difficult voices:** When no single 15s window contains a clean solo vocal (background music, multiple speakers, crowd noise), do NOT blindly extract the longest segment or use `clone-voice.sh` in non-interactive batch mode. The correct approach is: (1) download the full clip as WAV, (2) run faster-whisper across the whole file to get timestamped segments, (3) use a spectral mid/high-ratio heuristic to flag music-contaminated windows (ratio < ~4.0 indicates music), (4) extract only the clean solo-speaker windows (manually verifying transcripts to exclude multiple actors), apply noisereduce per chunk, add 30ms fades, concatenate with 150ms silence gaps, (5) write the spliced WAV directly to `voices/{name}-ref.wav`, (6) set `reference_text` to only the words in the spliced audio. The `segment_start_s` field in the JSON should reflect the earliest source segment used.

> [!TIP]
> **Agent Splicing Tool:** Agents should use `python scripts/splice-voice.py --wav <file> --voice <name> --match "<unique substring>" --reject-music` to automate this workflow. The script runs whisper, keeps segments containing the literal `--match` substring, applies the spectral heuristic, performs noise reduction and fading, and updates every JSON profile whose `reference_audio` points at `{name}-ref.wav` (the profiles must exist first; overwriting a git-tracked reference requires `--force`). It cannot verify speakers — review the printed `[KEPT]` transcript lines to confirm they are all the target actor. When downloading source audio, prefer highly targeted YouTube queries (e.g., `"Character Name specific quote"`) over generic character names to avoid multi-actor scenes.
...
**Mute convention:** `afterwords mute` toggles `/tmp/afterwords-muted`. Every **automatic** agent-response playback path guards `afplay` on this flag (`[ -f "$MUTE_FILE" ] || afplay …` in shell; `if not Path("/tmp/afterwords-muted").exists():` in `handler.py`) so muting silences the Mac while synthesis + archiving + feed delivery (Discord/Telegram) continue unaffected. Mute is intentionally scoped to the per-turn agent voice: **foreground, user-initiated** playback is exempt and still plays — `afterwords voices --demo`, the post-clone preview in `clone-voice.sh`, and the "say X in Y's voice" skill (`skill/scripts/speak.sh`) — because invoking them is an explicit request to hear audio. `tests/test_mute_guard.py` enforces this: it discovers every `afplay` site (no hand-maintained allowlist) and fails on a new unguarded one unless it is added to the test's `EXEMPT` set with a rationale.
